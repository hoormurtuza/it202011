...
$userID = get_user_id();
$db = getDB();
$stmt = $db->prepare("SELECT F20_Orders.id,OrderItems.product_id FROM F20_Orders JOIN OrderItems ON F20_Orders.id = OrderItems.order_id  where F20_Orders.user_id =:id  AND OrderItems.product_id = :product_id ");
