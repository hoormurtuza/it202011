...
if(isset($_POST["submit"])) {
    ...
    if ($valid == true && $payment != -1) {
        ...
        $db = getDB();
        $stmt = $db->prepare("INSERT INTO F20_Orders (user_id,total_price,address,payment_method) VALUES (:user,:total,:add,:pay)");
        $r = $stmt->execute([
            ":user" => $id,
            ":total" => $price,
            ":add" => $add,
	   ...
	   echo("There are 0 products available");

        }
        $id = get_user_id();
//Get last Order ID from F20_Orders table
        ...
        foreach ($order_items as $item) {
            ...
            $stmt = $db->prepare("INSERT INTO order_items (order_id, product_id, quantity, price)VALUES (:order_id, :pid,:q,:p)");
            $r = $stmt->execute([
                ":order_id" => $order_id,
                ":pid" => $product_id,
                ":q" => $item_quantity,
